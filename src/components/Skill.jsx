      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          {skillCategories.map(({ category, color, skills }) => (
            <div
              key={category}
              className="bg-[#172032] border border-[#1e3a5f] rounded-2xl p-4 sm:p-6 hover:border-[#38bdf8] transition-all duration-300"
            >
              {/* Category Label */}
              ...
              <div className="flex flex-wrap gap-1.5 sm:gap-2">
                {skills.map((skill) => (
                  <span
                    key={skill}
                    className="px-2.5 sm:px-3 py-1 sm:py-1.5 text-xs sm:text-sm font-medium rounded-lg bg-[#0f172a] text-slate-300 border border-[#1e3a5f] hover:text-white hover:border-slate-400 transition-all duration-200"
                  >
                    {skill}
                  </span>
                ))}
